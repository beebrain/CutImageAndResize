...
class FileAssert(object):
    ...
    def unpackDataset(self):
        if not (os.path.exists(self.detailData) and os.path.isfile(self.imageData)):
            ...
            imgDataset = imgDataset.reshape((-1,200,200,3))

            #Make sure dataset print out
            # for index in xrange(len(img)):
            #     cv2.imshow("x",img[index])
...
# imageDataset,detailDataset = FileAssert().packData()
#
# (x,y)=FileAssert().unpackDataset()
# class_label = y[:,0]
# personal = y[:,1]
# print len(x)
# rangeindex  = np.where(class_label==12)
